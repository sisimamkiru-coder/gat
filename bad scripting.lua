...
local function assignMyPlot()
    ...
    for i = 1, 8 do
        ...
        if plotFolder then
            ...
            if nameplate then
                local textLabel = nameplate.Nameplate.SurfaceGui.NameOf
...
if not myPlot then 
    myPlot = workspace.Plots.Plot_1 
    warn("Plot detection failed. Using Plot_1 as fallback.")
end
...
local Window = Rayfield:CreateWindow({
    Name = "My Knife FARM",
    LoadingTitle = "Version: 0.1",
    ConfigurationSaving = { Enabled = true, FolderName = "KnifeFarmAuto", FileName = "Config" }
})
                ...
                if next(selectedSwordRarity) or next(selectedMutationRarity) then
                    if isMatch(selectedSwordRarity, curR) and isMatch(selectedMutationRarity, curM) then
                        if autoBuyEnabled then
                            ...
                            StatusLabel:Set("Stopped At: " .. curM .. " " .. curR)
...
local rollToggle = Tab:CreateToggle({
    Name = "Auto Roll",
    CurrentValue = false,
    Flag = "caseRoll", 
    Callback = function(Value)
        ...
        if autoRollEnabled then
            StatusLabel:Set("Status: High-Speed Rolling...")
            task.spawn(function()
...
local sellToggle = SettingsTab:CreateToggle({
    Name = "Auto Sell",
    CurrentValue = false,
    Flag = "autoSell",
    Callback = function(Value)
            ...
            task.spawn(function()
                while autoSellEnabled do
                    ...
                    while timeLeft > 0 and autoSellEnabled do
                        ...
                        SellTimerLabel:Set(string.format("Next Sell In: %02d:%02d", mins, secs))
                        task.wait(1)
                        timeLeft = timeLeft - 1
                    end
                end
...
local intervalDropdown = SettingsTab:CreateDropdown({
    Name = "Sell Interval (Minutes)",
    Options = {"1", "5", "10", "20"},
    CurrentValue = "1",
    Callback = function(Option)
        local selectedValue = type(Option) == "table" and Option[1] or Option
        local num = tonumber(selectedValue)
...
local rarityDropdown = Tab:CreateDropdown({
    ...
    Options = {"Common", "Rare", "Epic", "Elite", "Legendary", "Mythic", "Secret", "Limited", "Exclusive", "Timeless", "Godly", "Soul", "Fruit", "Ninja", "Historical"},
...
local spinButton = SettingsTab:CreateButton({
   Name = "Spin Wheel",
   Callback = function()
